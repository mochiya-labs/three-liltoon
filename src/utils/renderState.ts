...
const stencilFunctionMap = {
	...
	2: LessStencilFunc,
	3: EqualStencilFunc,
	4: LessEqualStencilFunc,
	5: GreaterStencilFunc,
	6: NotEqualStencilFunc,
	7: GreaterEqualStencilFunc,
	8: AlwaysStencilFunc,
	Never: NeverStencilFunc,
	Less: LessStencilFunc,
	Equal: EqualStencilFunc,
	LEqual: LessEqualStencilFunc,
	Greater: GreaterStencilFunc,
	NotEqual: NotEqualStencilFunc,
	GEqual: GreaterEqualStencilFunc,
...
const stencilOperationMap = {
	...
	1: ZeroStencilOp,
	2: ReplaceStencilOp,
	3: IncrementStencilOp,
	4: DecrementStencilOp,
	5: InvertStencilOp,
	6: IncrementWrapStencilOp,
	7: DecrementWrapStencilOp,
	Keep: KeepStencilOp,
	Zero: ZeroStencilOp,
	Replace: ReplaceStencilOp,
	IncrSat: IncrementStencilOp,
	DecrSat: DecrementStencilOp,
	Invert: InvertStencilOp,
	IncrWrap: IncrementWrapStencilOp,
	DecrWrap: DecrementWrapStencilOp,
} as const;
...
export function applyLilToonRenderState(
	...
	material.side = mapped(
		cullMap as Record<string | number, Material["side"]>,
		cull,
		FrontSide,
	);
	material.depthWrite =
		numeric(properties, "_ZWrite", renderMode === "transparent" ? 0 : 1) !== 0;
	material.depthTest = true;
	material.depthFunc = mapped(
		depthMap as Record<string | number, Material["depthFunc"]>,
	...
	if (material.transparent) {
		...
		material.blendSrc = mapped(
			blendFactorMap as Record<string | number, Material["blendSrc"]>,
			(properties._SrcBlend as Mappable) ?? 5,
			SrcAlphaFactor,
		);
		material.blendDst = mapped(
			blendFactorMap as unknown as Record<
				string | number,
				Material["blendDst"]
			>,
			(properties._DstBlend as Mappable) ?? 10,
			OneMinusSrcAlphaFactor,
		);
		material.blendEquation = mapped(
			equationMap as Record<string | number, Material["blendEquation"]>,
			(properties._BlendOp as Mappable) ?? 0,
			AddEquation,
		);
		material.blendSrcAlpha = mapped(
			blendFactorMap as Record<
				string | number,
				NonNullable<Material["blendSrcAlpha"]>
			>,
			(properties._SrcBlendAlpha as Mappable) ?? 1,
			OneFactor,
		);
		material.blendDstAlpha = mapped(
			blendFactorMap as unknown as Record<
				string | number,
				NonNullable<Material["blendDstAlpha"]>
			>,
			(properties._DstBlendAlpha as Mappable) ?? 10,
			OneMinusSrcAlphaFactor,
		);
		material.blendEquationAlpha = mapped(
			equationMap as Record<
				string | number,
				NonNullable<Material["blendEquationAlpha"]>
			>,
			(properties._BlendOpAlpha as Mappable) ?? 0,
			AddEquation,
		);
	}
	material.stencilWrite =
		numeric(properties, "_StencilRef", 0) !== 0 ||
		numeric(properties, "_StencilWriteMask", 255) !== 255;
	...
	material.stencilFunc = mapped(
		stencilFunctionMap as Record<string | number, Material["stencilFunc"]>,
		(properties._StencilComp as Mappable) ?? 8,
		AlwaysStencilFunc,
	);
	material.stencilFail = mapped(
		stencilOperationMap as Record<string | number, Material["stencilFail"]>,
		(properties._StencilFail as Mappable) ?? 0,
		KeepStencilOp,
	);
	material.stencilZFail = mapped(
		stencilOperationMap as Record<string | number, Material["stencilZFail"]>,
		(properties._StencilZFail as Mappable) ?? 0,
		KeepStencilOp,
	);
	material.stencilZPass = mapped(
		stencilOperationMap as Record<string | number, Material["stencilZPass"]>,
		(properties._StencilPass as Mappable) ?? 0,
		KeepStencilOp,
	);
