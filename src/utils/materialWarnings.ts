...
export function shaderPropertyReferences(
	vertex: string,
	fragment: string,
): Set<string> {
	return new Set(
...
function textureGates(property: string): string[] | null {
	...
	if (
		["_BaseMap", "_BaseColorMap"].includes(property) ||
		property.startsWith("_Outline")
	)
		return null;
...
export function collectMaterialWarnings(
	...
	const enabled = (name: string): boolean =>
		firstComponent(properties[name]) !== 0;
	...
	for (const [property, texture] of Object.entries(textures).sort(([a], [b]) =>
		...
		if (
			gates === null ||
			gates.some((gate) => !enabled(gate) || unsupported.has(gate))
		)
			continue;
		if (property === "_MainColorAdjustMask") {
			const hsvg = properties._MainTexHSVG;
			const components = Array.isArray(hsvg)
				? hsvg
				: (hsvg as { toArray?: () => number[] } | undefined)?.toArray?.();
			if (
				!enabled("_MainGradationStrength") &&
				(!components ||
					components.every((value, i) => value === [0, 1, 1, 1][i]))
			)
				continue;
		}
		...
		const sharedNormal =
			/^_MatCap(?:2nd)?BumpMap$/.test(property) &&
			boundProperties.has("_BumpMap") &&
			texture === textures._BumpMap &&
		...
		const isCube = Boolean(
			(texture as Texture & { isCubeTexture?: boolean }).isCubeTexture,
		);
