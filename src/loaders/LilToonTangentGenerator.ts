...
function geometryCanGenerateTangents(geometry: BufferGeometry): boolean {
  return geometry.hasAttribute("position")
 ...
 */
export async function ensureLilToonTangents(meshes: readonly Mesh[]): Promise<void> {
  const geometries = new Set<BufferGeometry>();

  for (const mesh of meshes) {
    if (mesh.geometry.hasAttribute("tangent")) continue;
    const materials = Array.isArray(mesh.material) ? mesh.material : [mesh.material];
    const needsTangents = materials.some((material) =>
      material.isMaterial
