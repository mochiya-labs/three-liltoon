...
export class LilToonMorphAdapter {
	...
	update(mesh: Mesh, material: LilToonMaterial, renderer: WebGLRenderer): void {
		const positions = mesh.geometry.morphAttributes.position as
			MorphAttribute[] | undefined;
		const normals = mesh.geometry.morphAttributes.normal as
			MorphAttribute[] | undefined;
		...
		if (
			...
			entry.hasPositions !== hasPositions ||
			entry.hasNormals !== hasNormals
		) {
		...
		material.globalUniforms.uMorphTargetBaseInfluence = mesh.geometry
