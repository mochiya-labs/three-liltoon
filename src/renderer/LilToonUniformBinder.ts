...
type UniformPrimitive =
	number | Matrix4 | Vector2 | Vector3 | Vector4 | UniformPrimitive[];
...
function assignValue(
	current: UniformPrimitive,
	value: LilToonScalarOrVector | readonly number[],
): UniformPrimitive {
	const source =
		value instanceof Color ? [value.r, value.g, value.b, 1] : value;
...
function uploadTransposed(
	target: UniformPrimitive | undefined,
	source: Matrix4,
): void {
...
export function updateObjectCameraUniforms(
	...
	(globals.uCameraPosition as Vector4 | undefined)?.set(
		worldPosition.x,
		worldPosition.y,
		worldPosition.z,
		1,
	...
	(globals.uTime as Vector4 | undefined)?.set(
		elapsedSeconds / 20,
		elapsedSeconds,
		elapsedSeconds * 2,
		elapsedSeconds * 3,
	...
	const perspective =
		...
		Boolean((camera as { isPerspectiveCamera?: boolean }).isPerspectiveCamera);
	const near = (camera as { near?: number }).near ?? 0.1;
	const far = (camera as { far?: number }).far ?? 1000;
	(globals.uProjectionParams as Vector4 | undefined)?.set(
		1,
		near,
		far,
		far === 0 ? 0 : 1 / far,
	);
	(globals.uOrthoParams as Vector4 | undefined)?.set(
		0,
		0,
		0,
		perspective ? 0 : 1,
