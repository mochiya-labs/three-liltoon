...
describe("ShaderLab parser", () => {
	it("extracts the canonical lilToon material schema", () => {
		...
		expect(
			shader.properties.find((property) => property.name === "_Color"),
		).toMatchObject({
			type: "Color",
		...
		expect(
			shader.properties.find((property) => property.name === "_MainTex"),
		).toMatchObject({
			type: "2D",
			defaultValue: { texture: "white" },
		});
		expect(
			shader.properties.find((property) => property.name === "_Cutoff"),
		).toMatchObject({
			type: "Range",
			range: [-0.001, 1.001],
			defaultValue: 0.5,
	...
	it("extracts upstream opaque pass recipes and states", () => {
		const source = readFileSync(
			resolve(
				PROJECT_ROOT,
				"vendor/lilToon/Assets/lilToon/Shader/ltspass_opaque.shader",
			),
			"utf8",
		);
		const shader = parseShaderLab(source);
		const forward = shader.subShaders[0]?.passes.find(
			(pass) => pass.name === "FORWARD",
		);
