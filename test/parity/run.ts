...
function thresholdsFor(file: string): ParityThresholds {
  ...
  return { ...defaults, ...JSON.parse(readFileSync(path, "utf8")) as Partial<ParityThresholds> };
}
...
function rms(left: PNG, right: PNG): number {
  ...
  for (let index = 0; index < left.data.length; index += 4) {
    for (let channel = 0; channel < 3; channel += 1) {
      const delta = (left.data[index + channel]! - right.data[index + channel]!) / 255;
      squared += delta * delta;
      channels += 1;
    }
  }
...
if (references.length === 0) {
  process.stdout.write("No Unity parity references are installed. See test/parity/README.md.\n");
  ...
  for (const file of references) {
    ...
    if (reference.width !== output.width || reference.height !== output.height) {
      process.stderr.write(`${file}: size mismatch ${reference.width}x${reference.height} vs ${output.width}x${output.height}\n`);
      failures += 1;
      continue;
    }
    const settings = thresholdsFor(file);
    const diff = new PNG({ width: reference.width, height: reference.height });
    const different = pixelmatch(reference.data, output.data, diff.data, reference.width, reference.height, {
    ...
    const passed = ratio <= settings.maxDiffRatio && rmsError <= settings.maxRms;
