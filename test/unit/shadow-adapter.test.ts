import {
  DataTexture, DirectionalLight, Matrix4, Mesh, PerspectiveCamera,
  Scene, Vector4, WebGLRenderTarget, type Texture, type WebGLRenderer,
} from "three";
...
function shadowGlobals(): LilToonGlobalUniforms {
  return { uMainShadowMatrix: new Matrix4(), uShadowMapSize: new Vector4(), uShadowBias: 0, uShadowNormalBias: 0 };
}

function expectNeutral(texture: Texture | null, globals: LilToonGlobalUniforms): void {
  ...
  expect((globals.uMainShadowMatrix as Matrix4).equals(new Matrix4())).toBe(true);
  ...
  const depth = rgba[0] * 255 / 256 + rgba[1] * 255 / 65536 + rgba[2] * 255 / 16777216 + rgba[3] / 16777216;
...
function shadowTextures(material: LilToonMaterial): Texture[] {
  return Object.entries(material.uniforms).filter(([name]) => name.includes("uMainShadowMap")).map(([, uniform]) => uniform.value);
}

describe("directional shadow fallback", () => {
  it.each(["missing light", "casting disabled", "missing map", "renderer disabled"])("is unoccluded with %s and clears stale shadow state", reason => {
    ...
    const binding = adapter.bind(reason === "missing light" ? undefined : light, globals, reason !== "renderer disabled");
  ...
  it("preserves live shadow matrices, dimensions and biases, and restores them after re-enabling", () => {
    ...
    for (const enabled of [true, false, true]) {
      ...
      if (!enabled) { expectNeutral(binding.texture, globals); continue; }
      expect(binding.texture).toBe(light.shadow.map!.texture);
      expect((globals.uMainShadowMatrix as Matrix4).equals(originalMatrix.clone().transpose())).toBe(true);
      expect((globals.uShadowMapSize as Vector4).toArray()).toEqual([64, 32, 1 / 64, 1 / 32]);
  ...
  it.each(["opaque", "cutout", "transparent", "outline"] as const)("initializes and clears %s system shadow samplers with far depth", variant => {
    const material = new LilToonMaterial(variant === "outline" ? { pass: "outline" } : { renderMode: variant });
    ...
    for (const texture of textures) expect(Array.from((texture as DataTexture).image.data ?? [])).toEqual([255, 255, 255, 255]);
    const real = new DataTexture();
    material.setSystemTexture("__shadow", real);
    for (const texture of shadowTextures(material)) expect(texture).toBe(real);
    material.setSystemTexture("__shadow", null);
    for (const texture of shadowTextures(material)) expect(Array.from((texture as DataTexture).image.data ?? [])).toEqual([255, 255, 255, 255]);
    const clone = material.clone();
    for (const texture of shadowTextures(clone)) expect(Array.from((texture as DataTexture).image.data ?? [])).toEqual([255, 255, 255, 255]);
    material.dispose();
    clone.dispose();
    real.dispose();
  });
  ...
  it("honors renderer shadow toggles on successive draws without changing authored properties", () => {
    const renderer = { info: { render: { frame: 1 } }, shadowMap: { enabled: true } } as WebGLRenderer;
    const adapter = new LilToonRendererAdapter(renderer);
    const material = new LilToonMaterial({ properties: { _UseShadow: 1, _ShadowBorder: 0.4 } });
    const light = castingLight();
    const scene = new Scene();
    scene.add(light);
    for (const enabled of [true, false, true]) {
      renderer.shadowMap.enabled = enabled;
      adapter.prepareMaterial(material, renderer, scene, new PerspectiveCamera(), new Mesh(), 0);
      if (enabled) expect(shadowTextures(material)[0]).toBe(light.shadow.map!.texture);
