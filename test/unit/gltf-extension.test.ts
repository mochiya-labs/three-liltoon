...
describe("GLTFLilToonExtension", () => {
	...
	it("loads MatCap masks and independent normal-map transforms", async () => {
		...
		expect(lilToonMaterial.fragmentShader).toContain(
			"Combined_MatCapBlendMask",
		);
		expect(lilToonMaterial.fragmentShader).toContain(
			"Combined_MetallicGlossMap",
		);
		expect(
			(lilToonMaterial.globalUniforms._BumpMap_ST as Vector4).toArray(),
		).toEqual([2, 3, 0.1, 0.2]);
		expect(
			(lilToonMaterial.globalUniforms._Bump2ndMap_ST as Vector4).toArray(),
		).toEqual([7, 5, 0.3, 0.4]);
	});
