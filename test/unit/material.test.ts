...
describe("LilToonMaterial", () => {
	it("preserves lilToon property names and updates generated uniforms", () => {
		...
		expect((material.globalUniforms._Color as Vector4).toArray()).toEqual([
			0.8, 0.2, 0.3, 1,
		]);
	...
	it("converts linear shader output for an sRGB display", () => {
		...
		expect(material.fragmentShader).toContain(
			"uniform uint uLilToonOutputSrgb",
		);
	...
	it("uses the MatCap-mask shader profile and preserves independent normal transforms", () => {
		...
		expect((material.globalUniforms._BumpMap_ST as Vector4).toArray()).toEqual([
			2, 3, 0.1, 0.2,
		]);
		expect(
			(material.globalUniforms._Bump2ndMap_ST as Vector4).toArray(),
		).toEqual([7, 5, 0.3, 0.4]);
		expect(
	...
	it.each(["opaque", "cutout", "transparent"] as const)(
		...
		(renderMode) => {
			...
			for (const candidate of [material, clone]) {
				expect(
					(
						candidate.globalUniforms._ReflectionCubeTex_HDR as Vector4
					).toArray(),
				).toEqual([1, 1, 0, 0]);
			...
			expect(clone.globalUniforms._ReflectionCubeTex_HDR).not.toBe(
				material.globalUniforms._ReflectionCubeTex_HDR,
			);
		},
	);
	...
	it("preserves explicitly supplied material HDR decode parameters through updates and cloning", () => {
		const material = new LilToonMaterial({
			properties: { _ReflectionCubeTex_HDR: [2, 2.2, 0, 1] },
		});
		expect(
			(material.globalUniforms._ReflectionCubeTex_HDR as Vector4).toArray(),
		).toEqual([2, 2.2, 0, 1]);
		expect(
			(
				material.clone().globalUniforms._ReflectionCubeTex_HDR as Vector4
			).toArray(),
		).toEqual([2, 2.2, 0, 1]);
	...
	it("keeps Main Color 2nd with reflection controls when MatCap normals share the primary normal", () => {
		...
		expect(
			(material.globalUniforms._MatCapBumpMap_ST as Vector4).toArray(),
		).toEqual([2, 3, 0.1, 0.2]);
		expect(
			(material.globalUniforms._MatCap2ndBumpMap_ST as Vector4).toArray(),
		).toEqual([7, 5, 0.3, 0.4]);
	});
	...
	it("binds authored three-band shadow controls in the shadow-border profile", () => {
		...
		expect((material.globalUniforms._ShadowColor as Vector4).toArray()).toEqual(
			[0.59, 0.4, 0.4, 1],
		);
		expect(
			(material.globalUniforms._Shadow2ndColor as Vector4).toArray(),
		).toEqual([0.37, 0.24, 0.24, 1]);
		expect(
			(material.globalUniforms._Shadow3rdColor as Vector4).toArray(),
		).toEqual([0.14, 0.14, 0.14, 1]);
	});
	...
	it("serializes stable lilToon property names and caller-owned texture references", () => {
		...
		const texture = new DataTexture();
		material.setTexture("_MainTex", texture);
		const serialized = material.toLilToonJSON(
			(_value, property) => `texture:${property}`,
		);
	...
	it("preserves lilToon state when Three clones a material", () => {
		...
		expect((clone.globalUniforms._Color as Vector4).toArray()).toEqual([
			0.4, 0.5, 0.6, 0.7,
		]);
	});
