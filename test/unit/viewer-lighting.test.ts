...
const source = readFileSync(new URL("../../examples/mochiya-liltoon-viewer/src/components/viewer/viewer-lighting.tsx", import.meta.url), "utf8");
...
describe("viewer fixed lighting", () => {
  it("declares a fixed position without camera hooks or a moving target", () => {
    const position = lightElement.match(/position=\{\[([^\]]+)\]\}/)?.[1].split(",").map(Number);
  ...
  it("keeps the lilToon light direction and shadow view fixed during camera movement", () => {
    ...
    for (const cameraPosition of [[0, 2, 3], [3, 2, 0], [0, 2, -3], [-3, 2, 0], [12, 7, 25]]) {
      ...
      expect(light.target.getWorldPosition(new Vector3()).toArray()).toEqual([0, 0, 0]);
      expect(adapter.read(scene).direction.distanceTo(direction)).toBeLessThan(1e-8);
