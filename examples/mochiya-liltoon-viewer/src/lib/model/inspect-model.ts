...
function inspectMaterial(
	...
	return {
		...
		warnings:
			materialIndex == null
				? []
				: warnings.filter((warning) => warning.materialIndex === materialIndex),
	};
}
...
function materialsFromScene(parser: GLTFParser, scene: Object3D) {
	...
	scene.traverse((object) => {
		...
		for (const material of candidates) {
			...
			materials.set(
				index == null ? material.uuid : `material-${index}`,
				material,
			);
		}
	});
...
export function inspectModel(
	...
	const extensions = Array.isArray(gltf.parser.json.extensionsUsed)
		...
		: [];
	const materials = materialsFromScene(gltf.parser, gltf.scene).map(
		(material) => inspectMaterial(gltf.parser, material, warnings),
	);
