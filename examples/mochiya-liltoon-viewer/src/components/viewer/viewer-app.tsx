...
export default function ViewerApp() {
  ...
  const handleStatusChange = useCallback((next: LoadStatus) => setStatus(next), []);
      ...
      <header className="flex h-12 shrink-0 items-center justify-between gap-3 border-b bg-background px-3 sm:px-4">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <div className="min-w-0">
            <div className="truncate font-heading text-sm font-medium">lilToon model viewer</div>
            <div className="hidden text-[0.6875rem] text-muted-foreground sm:block">
              Three.js · VRM · Mochiya material inspection
            </div>
          </div>
          {inspection?.hasMochiyaLilToon && <Badge className="hidden sm:inline-flex">Extension loaded</Badge>}
        </div>

        <div className="flex shrink-0 items-center gap-1.5">
          <Button variant="outline" size="lg" onClick={openFilePicker}>
            <FileArrowUpIcon />
            <span className="hidden sm:inline">{source ? "Replace model" : "Open model"}</span>
