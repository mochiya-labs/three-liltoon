...
const manifest = Object.fromEntries(
	Object.entries(recipes).map(([renderMode, file]) => {
		const shader = parseShaderLab(
			readFileSync(resolve(LILTOON_SHADER, file), "utf8"),
		);
...
writeFileSync(
	resolve(GENERATED_SHADER_ROOT, "manifests/shaderlab-recipes.json"),
	`${JSON.stringify(manifest, null, 2)}\n`,
);
process.stdout.write(
	`Generated ShaderLab recipes for ${Object.keys(manifest).join(", ")}.\n`,
);
