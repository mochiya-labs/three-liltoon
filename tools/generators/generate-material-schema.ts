...
import { GENERATED_SOURCE_ROOT, LILTOON_ROOT, LILTOON_SHADER } from "../paths.js";
...
const textureSemantics = Object.fromEntries(
  properties
    ...
    .filter((entry): entry is [string, "color" | "normal" | "data"] => entry[1] !== undefined),
);
const commit = execFileSync("git", ["rev-parse", "HEAD"], { cwd: LILTOON_ROOT, encoding: "utf8" }).trim();
const description = execFileSync("git", ["describe", "--tags", "--always"], { cwd: LILTOON_ROOT, encoding: "utf8" }).trim();
const versionFile = JSON.parse(readFileSync(resolve(LILTOON_ROOT, "version.json"), "utf8")) as Record<string, unknown>;
const version = String(versionFile.latest_vertion_name ?? description);

const banner = "// Generated from vendor/lilToon/Assets/lilToon/Shader/lts.shader. Do not edit.\n";
...
process.stdout.write(`Generated ${properties.length} lilToon material properties from ${description}.\n`);
