...
if (
	pinned.dxc?.versionContains &&
	!versions.dxc.includes(pinned.dxc.versionContains)
) {
	throw new Error(
		`DXC version mismatch. Expected ${pinned.dxc.versionContains}.`,
	);
}
if (
	pinned.spirvTools?.versionContains &&
	!versions.spirvVal.includes(pinned.spirvTools.versionContains)
) {
	throw new Error(
		`SPIRV-Tools version mismatch. Expected ${pinned.spirvTools.versionContains}.`,
	);
}
