...
export function compileHlsl(options: DxcCompileOptions): void {
  ...
  const args = [
    ...
    "-fspv-reflect",
    "-O3",
    "-D", `SHADER_STAGE_${options.stage.toUpperCase()}=1`,
    "-I", resolve(PROJECT_ROOT, "shader/compat"),
    "-I", LILTOON_SHADER,
    "-I", LILTOON_INCLUDES,
    ...options.defines.flatMap((define) => ["-D", define]),
    ...(options.preprocessOutput ? ["-P", options.preprocessOutput] : ["-Fo", options.output]),
    options.source,
  ];
  execFileSync(options.executable, args, { cwd: PROJECT_ROOT, stdio: "inherit" });
}
