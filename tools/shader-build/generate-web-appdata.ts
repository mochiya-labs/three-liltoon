...
export function generateWebAppdata(): string {
  ...
  const occurrences = source.match(/\s+LIL_VERTEX_INPUT_INSTANCE_ID/g)?.length ?? 0;
  if (occurrences !== 2) {
    throw new Error(`Upstream appdata structure changed: expected 2 injection points, found ${occurrences}.`);
  }
  const generated = source.replace(/(\s+)LIL_VERTEX_INPUT_INSTANCE_ID/g, `${injection}$1LIL_VERTEX_INPUT_INSTANCE_ID`);
...
if (import.meta.url === `file://${process.argv[1]}`) process.stdout.write(`${generateWebAppdata()}\n`);
