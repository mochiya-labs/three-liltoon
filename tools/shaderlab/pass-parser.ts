...
function parseCompilerDirectives(
	source: string,
): Pick<ShaderLabPass, "defines" | "pragmas" | "includes"> {
	const defines: Record<string, string | boolean> = {};
	const pragmas: string[] = [];
	const includes: string[] = [];
	for (const line of source.split(/\r?\n/)) {
		const define =
			/^\s*#\s*define\s+([A-Za-z_][A-Za-z0-9_]*)(?:\s+(.+?))?\s*$/.exec(line);
...
export function parsePasses(
	subShaderSource: string,
	globalHlsl = "",
): ShaderLabPass[] {
	return findBlocksAtDepth(subShaderSource, "Pass", 0).map((block) => {
		const hlsl = [globalHlsl, extractHlsl(block.body)]
			.filter(Boolean)
			.join("\n");
