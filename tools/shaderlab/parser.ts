...
export function parseShaderLab(source: string): ShaderLabShader {
  ...
  if (!shaderMatch) throw new SyntaxError("ShaderLab source has no Shader declaration");
  const shaderBlocks = findBlocksAtDepth(source, "Shader", 0);
  if (shaderBlocks.length !== 1) {
    throw new SyntaxError(`Expected one Shader block, found ${shaderBlocks.length}`);
  }
