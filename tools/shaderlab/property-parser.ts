...
function parseNumber(value: string): number {
	...
	if (!Number.isFinite(parsed))
		throw new SyntaxError(`Invalid numeric default: ${value}`);
	return parsed;
}
...
function parseType(source: string): {
	...
	if (range) {
		return {
			type: "Range",
			range: [parseNumber(range[1]!), parseNumber(range[2]!)],
		};
	}
...
export function parseProperties(source: string): ShaderProperty[] {
	...
	for (const line of source.split(/\r?\n/)) {
		...
		if (!clean) continue;
		const match =
			/^((?:\[[^\]]+\]\s*)*)([_A-Za-z][_A-Za-z0-9]*)\s*\(\s*"((?:\\.|[^"])*)"\s*,\s*(.+)\)\s*=\s*(.+)$/.exec(
				clean,
			);
		if (!match) continue;
		const attributes = [...match[1]!.matchAll(/\[([^\]]+)\]/g)].map((item) =>
			item[1]!.trim(),
		);
