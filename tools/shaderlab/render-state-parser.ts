...
export function parseStencil(source: string): ShaderStencilState {
  ...
  for (const line of source.split(/\r?\n/)) {
    const match = /^\s*(Ref|ReadMask|WriteMask|Comp|Pass|Fail|ZFail)\s+(.+?)\s*$/.exec(line);
...
export function parseFlatRenderStates(source: string): ShaderRenderState {
  ...
  for (const line of stripped.split(/\r?\n/)) {
    const match = /^\s*(Cull|ZWrite|ZTest|BlendOp|Blend|Offset|ColorMask|AlphaToMask)\s+(.+?)\s*$/.exec(
      line,
    );
    if (!match) continue;
    const [, key, value] = match;
    switch (key) {
      case "Cull": state.cull = parseStateValue(value!); break;
      case "ZWrite": state.zWrite = parseStateValue(value!); break;
      case "ZTest": state.zTest = parseStateValue(value!); break;
      case "Blend": state.blend = splitStateValues(value!); break;
      case "BlendOp": state.blendOp = splitStateValues(value!); break;
      case "Offset": state.offset = splitStateValues(value!); break;
      case "ColorMask": state.colorMask = parseStateValue(value!); break;
      case "AlphaToMask": state.alphaToMask = parseStateValue(value!); break;
    }
  }
  return state;
}
